...
def find_available_room(
    ...
    for room in rooms:
        if room.capacity < required_capacity:
            continue
        if course.requires_computers and room.computers < required_capacity:
            continue
    ...
    for class_group in sorted(course.classes, key=lambda c: c.name.lower()):
        ...
        for day in priority_days:
            ...
            for slot_start_time in START_TIMES:
                ...
                if not teacher:
                    continue
                required_capacity = course.expected_students_for(class_group)
                room = find_available_room(course, start_dt, end_dt, required_capacity)
                if not room:
                    continue
                session = Session(
                    course=course,
